...
export async function invokeProtectionScopeApi(accessToken: string): Promise<{ body: any; etag: string | null }> {
  ...
  export async function EnqueueOfflinePurviewTasksAsync(
    ...
    try {
      if (uploadTextMode === 'evaluateOffline')  {
            context.log('Handling evaluateOffline logic for uploadText...');    
             ...
             // Call the second API
             const { body: processContentResponse, headers:responseHeaderPrompt } = await invokeProcessContentApi(accessToken,etag, processContentRequestBody);
            ...
            if (downloadTextMode === 'evaluateOffline') {
             ...
             const processContentRequestBodyForLLM = constructProcessContentRequestBody(
              ...
              const { body: processContentResponseForLLM, headers: responseHeadersForLLM }  = await invokeProcessContentApi(accessToken, etag, processContentRequestBodyForLLM);
    ...
    return "OK"; // Assuming the status text is "OK" for successful responses
        ...
        context.error(`Error invoking ProcessContent API for ${error}:`);
      throw error;
    }
  }
  ...
  export function constructProcessContentRequestBody(
    ...
    return {
      contentToProcess: {
        ...
        activityMetadata: {
        activity: activity, // Example activity
        },
        deviceMetadata: {
          deviceType: "managed",
